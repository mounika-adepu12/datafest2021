...
# FINAL MODEL OVERALL: all regions combine
...
glance(best_model_abuse_given_use) %>% 
...
Out of the respondents who have used presciption drugs during their lifetime, we found that age was a significant predictor of the odds of a respondent having misused prescription drugs in their lifetime. For each additional year in age, the odds of a respondent having misused prescription drugs are expected to multiply by a factor of `r round(exp(-0.045126773), 3)` (p < 0.05), holding all other predictor variables constant. This may suggest that younger individuals are more likely to misuse the drug compared to older individuals, who are less likely to misuse their prescribed drugs.

As we would expect, another key demographic that influences prescription drug misuse is gender. The odds of misuse for individuals who are female are expected to multiply by a factor of `rround(exp(-0.535022131), 3)` (p < 0.05) compared to individuals who are males, holding all else constant. 

Another significant covariate of drug misuse is income. In comparison to individuals in the lowest income bracket (Less than 25,000 dollars), the odds of misuse for individuals who have an income of $100,000 or more are expected to multiply by a factor of `rround(exp(-0.208111158), 3)` (p < 0.05), holding all else constant.

Finally, a significant predictor is region. The odds of misuse for individuals who are in the West (Region4) are expected to multiply by a factor of `rround(exp(0.101726132	), 3)` (p < 0.05) compared to individuals who are in the North East (Region1), holding all else constant. On the other hand, the odds of misuse for individuals who are in the MidWest (Region2) are expected to multiply by a factor of `rround(exp(-0.220263014), 3)` (p < 0.05) compared to individuals who are in Region1 (the North East), holding all else constant.

According to the model, we would tell drug prescribers that young males of lower economic status are the population under the highest risk of misusing prescribed drugs.

As a result of differences by region, we were curious to explore the predictors of drug misuse by region. That is, what is the profile of a person who misuses drugs by U.S. region?
...
```{r augment-for-roc}
best_model_abuse_given_use_aug <- augment(best_model_abuse_given_use, type.predict = "response")
```
...
roc_curve_data <- best_model_abuse_given_use_aug %>%
  roc_curve(ALL_NMU, .fitted) %>%
...
#roc_curve_data %>%
#  filter(.threshold > 0.05666995, .threshold < 0.05672580) %>% 
...
ggplot(ALL_REGIONS_TIDY, aes(y = estimate, x = term, color = REGION))+
  ...
  coord_flip() +
  theme_bw() +
  labs(x = "Covariates of Interest", y = "Estimate", title = "Predictors of Drug Misuse", subtitle = "By US Region")
dev.off()
```
